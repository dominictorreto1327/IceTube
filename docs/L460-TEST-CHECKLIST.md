...
- [ ] mpv 能同时播放画面和声音
- [ ] 主窗口始终保留 16:9 黑色播放区，播放时没有独立 mpv 窗口
- [ ] 16:9 视频填满播放区；4:3、竖屏视频只留左右黑边；超宽视频只留上下黑边（源文件自带黑边除外）
- [ ] 调整窗口大小、最大化和还原后，播放区仍为 16:9，画面完整等比显示
- [ ] 停止和自然播放结束后恢复黑屏，再次播放仍在同一区域显示
- [ ] 播放时更换视频、解析时停止再播放，不出现旧任务状态覆盖新任务
